...
function isLocal(event, anchor, lookForHash) {
    ...
    // Some browsers return an empty string for href="#"
    // so we also check the href
...
function getEventAndAnchor() {
    ...
    if (!obj.anchor && obj.event && isHTMLElement(obj.event.target)) {
        obj.anchor = obj.event.target;
    }
